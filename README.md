...
- Moving between pages uses cross-document View Transitions, CSS only. The old page is gone in 120ms and the new one rises 6px in over 280ms from 60ms. Only the small hue dot is paired by slug between the landing entry and post; titles, nets and the nav tab do not travel. Same-origin pages are prerendered on hover or focus (Chromium).
